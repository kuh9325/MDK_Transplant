...
| Save/load | ABSENT | PROVEN — envelope+packet model closed (Phase 14B): native reader validates all 5 real saves, header-only writer + `SaveStore`, death→`LASTGAME.SAV`→Continue loop native-closed in `ProgressionSession`; full-save world application (AREN/ALIE restore) remains the bounded seam — see `docs/reverse-engineering/SAVE_FORMAT.md` | us | bounded RE for full-save restore |
